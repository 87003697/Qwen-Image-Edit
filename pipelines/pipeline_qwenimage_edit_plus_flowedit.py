...
class QwenImageEditPlusPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def __call__(
        ...
        print(f"DEBUG: timesteps={timesteps[:5]}...{timesteps[-5:]}")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print(f"DEBUG: i={i}, t={t:.2f}, dt={dt:.4f}, |v_src|={noise_pred_src.norm().item():.2f}, |v_tgt|={noise_pred_tgt.norm().item():.2f}, |v_delta|={v_delta.norm().item():.4f}, z_edit_diff={(z_edit - x_src).norm().item():.4f}")
