...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers import QwenImageEditFlowEditPipeline
        >>> from diffusers.utils import load_image

        >>> pipe = QwenImageEditFlowEditPipeline.from_pretrained("Qwen/Qwen-Image-Edit", torch_dtype=torch.bfloat16)
        >>> pipe.to("cuda")
        >>> image = load_image(
        ...     "https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/diffusers/yarn-art-pikachu.png"
        ... ).convert("RGB")
        >>> source_prompt = "A yarn art pikachu"
        >>> prompt = (
        ...     "Make Pikachu hold a sign that says 'Qwen Edit is awesome', yarn art style, detailed, vibrant colors"
        ... )
        >>> # Depending on the variant being used, the pipeline call will slightly vary.
        >>> # Refer to the pipeline documentation for more details.
        >>> image = pipe(image, prompt=prompt, source_prompt=source_prompt, num_inference_steps=50).images[0]
        >>> image.save("qwenimage_edit_flowedit.png")
        ```
"""
...
class QwenImageEditFlowEditPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    r"""
    The Qwen-Image-Edit pipeline for image editing with FlowEdit.
    ...
    def __call__(
        ...
        source_image: PipelineImageInput,
        prompt: Union[str, List[str]] = None,
        ...
        Args:
            ...
            source_image (`torch.Tensor`, `PIL.Image.Image`, ...):
                Source Reference Image. Must be provided.
            prompt (`str` or `List[str]`, *optional*):
        ...
        # 3. Preprocess images (target & source fully separated)
        if image is None:
            raise ValueError("`image` (target reference) must be provided.")
        if source_image is None:
            raise ValueError("`source_image` must be provided.")

        # Target prompt image (visual context for target prompt) + VAE input for target condition
        if not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            image_tgt_resized = self.image_processor.resize(image, calculated_height, calculated_width)
            prompt_image_tgt = image_tgt_resized
            vae_input_tgt = self.image_processor.preprocess(image_tgt_resized, calculated_height, calculated_width)
            vae_input_tgt = vae_input_tgt.unsqueeze(2)
        else:
            # latent input case
            prompt_image_tgt = None
            vae_input_tgt = image

        # Source prompt image (visual context for source prompt) + VAE input for source condition
        if not (isinstance(source_image, torch.Tensor) and source_image.size(1) == self.latent_channels):
            image_src_resized = self.image_processor.resize(source_image, calculated_height, calculated_width)
            prompt_image_src = image_src_resized
            vae_input_src = self.image_processor.preprocess(image_src_resized, calculated_height, calculated_width)
            vae_input_src = vae_input_src.unsqueeze(2)
        else:
            # latent input case
            prompt_image_src = None
            vae_input_src = source_image
        ...
        # Encode Source Prompt (with Source Image)
        prompt_embeds_src, prompt_embeds_mask_src = self.encode_prompt(
            image=prompt_image_src,
        ...
        # Encode Target Prompt (with Target Image)
        prompt_embeds_tgt, prompt_embeds_mask_tgt = self.encode_prompt(
            image=prompt_image_tgt,
            prompt=prompt,
            prompt_embeds=prompt_embeds,
            prompt_embeds_mask=prompt_embeds_mask,
            device=device,
        ...
        # Prepare Target Latents (Condition) & Noise
        latents, image_latents_tgt = self.prepare_latents(
            vae_input_tgt,
        ...
        # Prepare Source Latents (Condition & Start Point)
        # Note: We don't need new noise for source, so passing latents=latents just to reuse shape logic, 
        # but we only care about image_latents_src return.
        _, image_latents_src = self.prepare_latents(
            vae_input_src,
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
            ...
            prompt_embeds_src.dtype,
            device,
            generator, # Same generator or different? Usually deterministic doesn't matter for condition encoding
            latents,
        )
        
        # x_src should be the clean latents of the SOURCE image.
        x_src = image_latents_src.clone()
        z_edit = x_src.clone()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Source Model Input: Concat with SOURCE Condition
                latent_model_input_src = torch.cat([latents_src, image_latents_src], dim=1)
                ...
                with self.transformer.cache_context("cond"):
                    ...
                    noise_pred_src = noise_pred_src[:, : x_src.size(1)]
                ...
                if do_true_cfg_src:
                    with self.transformer.cache_context("uncond"):
                        ...
                        neg_noise_pred_src = neg_noise_pred_src[:, : x_src.size(1)]
                ...
                # Target Model Input: Concat with TARGET Condition
                latent_model_input_tgt = torch.cat([latents_tgt, image_latents_tgt], dim=1)
                ...
                with self.transformer.cache_context("cond"):
                    ...
                    noise_pred_tgt = noise_pred_tgt[:, : x_src.size(1)]
                ...
                if do_true_cfg_tgt:
                    with self.transformer.cache_context("uncond"):
                        ...
                        neg_noise_pred_tgt = neg_noise_pred_tgt[:, : x_src.size(1)]
                ...
                latents = z_edit # For callback and return
